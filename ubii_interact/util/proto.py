...
class Translator(ProtoMessageFactory[GPM]):
    ...
    def create(cls, *args, **kwargs) -> GPM:
        ...
        If keyword and positional arguments are used, it produces Messages by creating new objects of this classes
        `proto` type and initializing them with the supplied arguments.
        If only keyword arguments are used, it can instead use `from_dict`, i.e. the names of the
        keyword arguments are not case sensitive and can be supplied as camel case as well as pascal case.
    ...
    def generate_translator(datatype: str) -> 'Translator':
        ...
        if not datatype.startswith('ubii.'):
            log.debug(f"Type {datatype} is not a protobuf type. Generating Fake Translator")
        else:
