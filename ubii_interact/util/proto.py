...
class Translator(ProtoMessageFactory[GPM]):
    ...
    @classmethod
    def validate(cls, message: GPM, *args, **kwargs):
...
class Proto(Generic[GPM]):
    ...
    async def create(cls, *args, **kwargs):
        instance = cls()
        try:
            await instance._async_init(*args, **kwargs)
...
# ------------------ Wrapper classes -------------------------
class ProcessingModule(Proto, translator=Translators.PM):
...
class Session(Proto, translator=Translators.SESSION):
    id = Proto.ProtoProperty()
    name = Proto.ProtoProperty()
...
class Component(Proto, translator=Translators.COMPONENT):
